=begin
	...
	Scene_HighscoreEntry1: Player 1 high score record

=end

class Scene_HighscoreEntry1
	
	def initialize(window)
		@window = window
		@background = Image.new(@window, "Graphics/Backgrounds/HSName_P1.gif", false)
		@cursor = Image.new(@window, "Graphics/Sprites/Cursor2.gif", false)
		@cursor_x = 165
		@cursor_y = 300
		@cursor_index = 0
		@bgm = Song.new(window, "Audio/BGM/GameOver.ogg")
		@bgm.play(true)
	...
	def draw
		...
		$font.draw("#{$score.to_s}", 223, 215, 5, 1.0, 1.0, 0xffffff00)
	...
	def confirm
		...
		if $twoplayers  == true
			# Generate high score
			@generate.generate_score(text,$score)
			# Change scene to player 2
			$scene = Scene_HighscoreEntry2.new($window)
		else
